...
- `data.lua`：data 阶段创建同分类隐藏资源副本、临时放置锚点资源和大矿机放置代理。
- `data-final-fixes.lua`：data-final-fixes 阶段给原版大矿机开启资源筛选槽，并把物品放置结果改为代理实体。
- `control.lua`：control 阶段把放置代理替换为原版大矿机，跟踪传奇大矿机，并把玩家选择的原版资源筛选器映射到隐藏资源副本。
- `constants.lua`：跨阶段常量。
...
- 玩家放置原版 `big-mining-drill` 物品时，实际先落地一个不需要资源的代理实体；control 阶段再按品质和资源规则替换为原版矿机。
